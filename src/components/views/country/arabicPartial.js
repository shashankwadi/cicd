...
export default class ArabicPartial extends PureComponent {
    ...
    _onPressArabic = () => {
        this.props.onPressArabic(); // callback to parent view to set
    };

    _onPressEnglish = () => {
        this.props.onPressEnglish();
    };
    ...
    render() {
        ...
        if(GLOBAL.CONFIG.enableMobileWeb){
            ...
            containerViewTopMargin = (this.props.isIphoneX)?70:20;
        }
        ...
        return (
            <View style={[{flex: 1, backgroundColor: GLOBAL.COLORS.screenBackgroundGray}]}>
                ...
                <Text style={styles.selectLanguageText}>
                    {text.ar.choose_your_language}
                </Text>

                <View style={{height: 50, flexDirection: 'row'}}>
                    <View style={{flex: .5, justifyContent: 'center'}}>
                        <TouchableOpacity activeOpacity={1}
                                          style={[styles.engLngSelectionButtonStyle, {backgroundColor: engBgColor}]}
                                          onPress={this._onPressEnglish}>
                            <Text style={{color: engTxtColor, textAlign: 'center'}}>
                                {text.ar.english.toUpperCase()}
                            </Text>
                        </TouchableOpacity>
                    </View>
                    <View style={{flex: .5, justifyContent: 'center'}}>
...

                {
                    ...
                    <FlatList
                        ...
                        keyExtractor={(item, index) => {
                            return `arabicCountries-${item}-${index}`
                        }}
